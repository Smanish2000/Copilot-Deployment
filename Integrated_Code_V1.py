...
if 'comparison_view' not in st.session_state:
    st.session_state['comparison_view'] = None
if 'devices_comparison' not in st.session_state:
    st.session_state['devices_comparison'] = [None, None]
...
def identify_devices(input_string):
    ...
    df = pd.read_csv('Windows_Data_116K_V2.csv')
...
def get_conversational_chain_quant(history):
    try:
        ...
        "Summarize reviews of CoPilot Produt" - User seeks for net sentiment and aspect wise net sentiment of "CoPilot" Product and their respective review count in a single table
        "Summarize reviews of CoPilot Produt in US Geography" - User seeks for net sentiment and aspect wise net sentiment of "CoPilot" Product and their respective review count in a single table but only from 'US' Geography
...
#################################################################################################################################################################################################################################################
        prompt_template = """
                ...
                There is only one table with table name Copilot_Sentiment_Data where each row is a user review. The table has 10 columns, they are:
                        Review: Review of the Copilot Product
                        Data_Source: From where is the review taken. It contains different retailers
                        Geography: From which Country or Region the review was given. It contains different Geography. The user might mention Geography as Geography/Geographies/Regions or nothing. Just the values inside it.
                        list of Geographies in the table - China,France,Japan,US,Brazil,Canada,Germany,India,Mexico,UK,Australia,Unknown,Venezuela,Vietnam,Cuba,Colombia,Iran,Ukraine,Northern Mariana Islands,Uruguay,Taiwan,Spain,Russia,Bolivia,Argentina,Lebanon,Finland,Saudi Arabia,Oman,United Arab Emirates,Austria,Luxembourg,Macedonia,Puerto Rico,Bulgaria,Qatar,Belgium,Italy,Switzerland,Peru,Czech Republic,Thailand,Greece,Netherlands,Romania,Indonesia,Benin,Sweden,South Korea,Poland,Portugal,Tonga,Norway,Denmark,Samoa,Ireland,Turkey,Ecuador,Guernsey,Botswana,Kenya,Chad,Bangladesh,Nigeria,Singapore,Malaysia,Malawi,Georgia,Hong Kong,Philippines,South Africa,Jordan,New Zealand,Pakistan,Nepal,Jamaica,Egypt,Macao,Bahrain,Tanzania,Zimbabwe,Serbia,Estonia,Jersey,Afghanistan,Kuwait,Tunisia,Israel,Slovakia,Panama,British Indian Ocean Territory,Comoros,Kazakhstan,Maldives,Kosovo,Ghana,Costa Rica,Belarus,Sri Lanka,Cameroon,San Marino,Antigua and Barbuda
                        Title: What is the title of the review
                        Review_Date: The date on which the review was posted
...
If user is seeking for any questions related to net sentiment and aspect sentiment choose Quantifiable and visualization
    ...
    Donot choose Quantifiable and visualization for Generic Questions
...
We have collected the reviews from consumer websites. So reviews are people thoughts and views about the Product.
...
2. Generic - This category allows users to ask general questions about any Product, such as the Pros and Cons, common complaints consumer faces which they post as reviews associated with a device, and the top verbatims (Reviews) mentioned in product reviews, etc.
    ...
    User Queries which are not related to Net Sentiment, Aspect sentiment, Summarization of one product, Comparison of 2 Products will come here.
    if user seeks the thoughts of people or reviwes from the consumer, choose this one.
    It also includes Examples : 
    1. "What do people think about pricing of Windows Copilot in US" etc.,
    2. "Why the sentiment is positive or negetive for any feature of a product.
...
4. Comparison - This feature allows users to compare two different Products based on user reviews. Remember that this function only does comparision for 2 different Products.
    Product Families - ["Windows Copilot" , "Microsoft Copilot" , "Github Copilot" , "Copilot Pro" , "Copilot for Security" , "Copilot for Mobile", "Copilot for Microsoft 365"]
...
Features and sample prompts:
    ...
    2. Summarization of reviews - "Summarize the reviews for [Product] / Analyze consumer reviews for [Product]
        Even if user mention anyother Geography - "Summarize reviews for [Product] in US Geography". You output should be "Summarize the reviews for [Product]"
    ...
    3. Asking net sentiment/aspect sentiment/ review count  - "What is the net sentiment and review count for [product 1]". It might also be "What is the best device?" (Based on Net Sentiment) , "What is the device user mostly comment about" (Based on Review Count) and all of these
        ...
        3.3 Whenever the user specifies any Geography. Let it be specifies in rephrased Qn as well. If the user asks for "Give me the net sentiment of Github Copilot in US Geography". Rephrased Question should be "What is the net sentiment and Review Count of Github Copilot in US Geography"
        3.4 Make sure you give one of the below Geographies : list of Geographies in the table - 
        [China,France,Japan,US,Brazil,Canada,Germany,India,Mexico,UK,Australia,Unknown,Venezuela,Vietnam,Cuba,Colombia,Iran,Ukraine,Northern Mariana Islands,Uruguay,Taiwan,Spain,Russia,Bolivia,Argentina,Lebanon,Finland,Saudi Arabia,Oman,United Arab Emirates,Austria,Luxembourg,Macedonia,Puerto Rico,Bulgaria,Qatar,Belgium,Italy,Switzerland,Peru,Czech Republic,
        Thailand,Greece,Netherlands,Romania,Indonesia,Benin,Sweden,South Korea,Poland,Portugal,Tonga,Norway,Denmark,Samoa,Ireland,Turkey,Ecuador,Guernsey,Botswana,Kenya,Chad,Bangladesh,Nigeria,Singapore,
        Malaysia,Malawi,Georgia,Hong Kong,Philippines,South Africa,Jordan,New Zealand,Pakistan,Nepal,Jamaica,Egypt,Macao,Bahrain,Tanzania,Zimbabwe,Serbia,Estonia,Jersey,Afghanistan,Kuwait,Tunisia,Israel,Slovakia,Panama,British Indian Ocean Territory,Comoros,Kazakhstan,Maldives,Kosovo,Ghana,Costa Rica,Belarus,Sri Lanka,Cameroon,San Marino,Antigua and Barbuda]
...
def user_ques(user_question, raw_user_question):
    global full_response
    if user_question:
    ...
    # Convert user question and product family names to lowercase for case-insensitive comparison
        ...
        if device_a != None and device_b != None:
            try:
                ...
                try:
                    raw_user_question = raw_user_question.replace("Compare", "Summarize reviews of")
                except:
                    pass
                comparision_summary = query_detailed_compare(raw_user_question + "Which have the following sentiment data" + str(c)+str(d))
